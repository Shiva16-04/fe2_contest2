...
let aToZButton=document.getElementById('btn1').addEventListener("click", aToZSorting)
let zToAButton=document.getElementById('btn2').addEventListener("click", zToASorting);
let byMarksButton=document.getElementById('btn3').addEventListener("click", byMarksSorting);
let byPassingButton=document.getElementById('btn4').addEventListener("click", byPassingSorting);
let byClassButton=document.getElementById('btn5').addEventListener("click", byClassSorting);
let byGenderButton=document.getElementById('btn6').addEventListener("click", byGenderSorting);
let searchButton = document.getElementById("btn-search").addEventListener("click", filterThroughSearchBar)
let searchBar = document.getElementsByTagName("input")[0].addEventListener("input", filterThroughSearchBar)

let studentData = fetchData();
window.onload = function() {
    let table = createTable()
    renderTable(studentData, table);
};
...
function createTable(){
    ...
    let headerArr = ["ID", "Name", "Gender", "Class","Marks","Passing","Email"]
    headerArr.forEach((headerText, index) => {
...
function renderTable(studentData, table) {
    console.log("adding rows or rendering table")
    ...
    studentData.forEach(student => {
        ...
        cell2.innerText=student.gender;

        const cell3=newRow.insertCell(3);
        cell3.innerText=student.class;

        const cell4=newRow.insertCell(4);
        cell4.innerText=student.marks;

        let cell5=newRow.insertCell(5);
        if(student.passing==true)cell5.innerText="Passed";
        else cell5.innerText="Failed";
        
        const cell6 =newRow.insertCell(6);
        cell6.innerText=student.email;
...
function byPassingSorting(){
    container.innerHTML='';
    let table = createTable()
    let newStudentData=studentData.filter(student=>student.passing==true)
    renderTable(newStudentData , table)
}
function byGenderSorting(){
    container.innerHTML='';

    let table1 = createTable();
    let newStudentData1 = sortByGenre("Female")
    renderTable(newStudentData1, table1)

    console.log("Gender Sorting2")

    let table2 = createTable();
    let newStudentData2 = sortByGenre("Male")
    renderTable(newStudentData2 , table2)
}
function sortByGenre(gender){
    let newStudentData=studentData.filter(student=>student.gender==gender)
    return newStudentData;
}

function filterThroughSearchBar(eventDetails){
    let search = searchBar.value;
    let searchString = search.trim();
    console.log(searchString)
    container.innerHTML='';
    let table = createTable();
    let newStudentData = filterBySearchString(searchString)
    renderTable(newStudentData , table)
}
function filterBySearchString(searchString){
    ...
    studentData.forEach(student=>{
        ...
        if(cnt>0)newStudentData.push(student)
    })
    return newStudentData;
}
